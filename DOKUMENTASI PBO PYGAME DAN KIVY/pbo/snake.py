...
class game():
    ...
    def over(self): #methode ini digunakan me while untuk posisi score ukuran font,serta ukuran tombol
        while 1:
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        self.screen.blit(txts,txtrect)
    def reset(self): #digunakan untuk
        self.left,self.right,self.up,self.down = False,False,False,False
    def loop(self): #method ini digunakan untuk melooping semua perintah seperti perintah up,down,left,right dan laain lain yang masih terkait
        ...
        while self.game_over != True:
            ...
            pg.display.update()
class startmenu():
    def __init__(self): #method ini digunakan untuk menginitkan ukuran layar,menu,tombol,border block,posisi border block
        self.screen = pg.display.set_mode((800,450))
        self.b1 = '(150, 300,100,50),"Maen", [(0,255,0), (0,150,0)], action = self.n'
        ...
        for x in range(0,450,10):
            ...
            t.fill(color)
            self.blocks.append([t, [790,x]])
    def make_text(self, x, y, text, size=20, color = (0,0,0), a = False): #digunakan untuk membuat type font ukuran font warna font letak font
        ...
        if a == True:
            txtrect.center = (x,y)
        self.screen.blit(txts, txtrect)
    def make_button(self, pos, text, color, action=None, textsize=20):#method digunakan untuk membuat posisi tombol,bentuk tombol, warna dan mengarahkan kursor
        mouse = pg.mouse.get_pos()
        oldpos = pos
        rect = pg.Rect(pos)
        pos = rect.topleft
    ...
    def mainloop(self): #METHODE YANG DIGUNAKAN UNTUK MENGULANG PEMANGGILAN DARI ATRIBUT TEXT,SCREEN,SERTA BUTTONS SEPERTI DIBAWAH INI(perulangan game)
        while 1:
            self.screen.fill((0,191,255))
            self.make_text(400, 150, '__ULO-ULONAN__', color = (255,255,255), size = 80, a = True)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    sys.exit()
            for x in self.blocks:
